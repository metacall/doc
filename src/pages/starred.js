...
const StarredPage = () => {
  ...
  const allDocs = Object.values(allDocsData).flatMap(
    (versionData) => versionData.versions.flatMap(v => v.docs)
  );
  ...
  return (
    <Layout title="Starred Docs" description="Your starred documentation">
      <div className="container margin-vert--xl">
        <h1>⭐ Starred tutorials</h1>
        <div className="row">
          {starredDocs.length > 0 ? (
            starredDocs.map((doc, index) => {
              const permalink = doc.permalink || `${baseUrl}docs/${doc.unversionedId || doc.id}`;
                ...
                <div className="col col--6 margin-bottom--md" key={index}>
                  <div className="card">
                    <div className="card__body" style={{
                      ...
                      >
                         {doc.id.split('/').pop().charAt(0).toUpperCase() + doc.id.split('/').pop().slice(1)}
                      </Link>
                    </div>
                  </div>
                </div>
            ...
            <p>No documents starred yet. Start starring docs to see them here!</p>
          )}
        </div>
      </div>
    </Layout>
